...
public final class Vulture_Not_Full extends EntityAb implements AnimationEntity, ActivityEntity {
    ...
    public Vulture_Not_Full(String id, Point position, List<PImage> images, int resourceLimit, int resourceCount, double actionPeriod, double animationPeriod) {
    ...
    public void executeActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<EntityAb> target = world.findNearest(this.getPosition(), new ArrayList<>(Arrays.asList(House.class)));
    ...
    private boolean transformNotFull(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        if (this.resourceCount >= this.resourceLimit) {
            Vulture_Full vulture = Functions.createVultureFull(this.getId(), this.getPosition(), this.actionPeriod, this.animationPeriod, this.resourceLimit, this.getImages());
